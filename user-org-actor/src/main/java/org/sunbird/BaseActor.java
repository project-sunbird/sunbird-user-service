  ...
  public void onReceive(Object message) throws Throwable {
    if (message instanceof Request) {
      ...
      try {
        ProjectLogger.log(
            String.format(
                "%s:%s:method started at %s",
                this.getClass().getSimpleName(), operation, System.currentTimeMillis()),
            LoggerEnum.DEBUG);
        onReceive(request);
        ProjectLogger.log(
            String.format(
                "%s:%s:method ended at %s",
                this.getClass().getSimpleName(), operation, System.currentTimeMillis()),
            LoggerEnum.DEBUG);
      } catch (Exception e) {
      ...
      ProjectLogger.log(
          "BaseActor: onReceive called with invalid type of request.", LoggerEnum.INFO);
    }
  }
