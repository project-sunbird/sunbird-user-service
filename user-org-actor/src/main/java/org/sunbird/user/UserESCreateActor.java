...
public class UserESCreateActor extends BaseActor {
    ...
    public void onReceive(Request request) throws Throwable {
        if (UserActorOperations.USER_ES_CREATE.getOperation().equalsIgnoreCase(request.getOperation())) {
            create(request);
        } else {
    ...
    private void create(Request request) throws BaseException{
        try {
            ...
            ProjectLogger.log("UserESCreateActor:create : Exception occurred while creating user in Elastic search. ",ex);
            throw new ESException.SaveException(IResponseMessage.ES_SAVE_ERROR,localizer.getMessage(IResponseMessage.ES_SAVE_ERROR,null), ResponseCode.SERVER_ERROR.getCode());
        }
    }
}
