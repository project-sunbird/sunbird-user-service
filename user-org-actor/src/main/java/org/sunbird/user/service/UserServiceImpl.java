...
public class UserServiceImpl implements IUserService {
  ...
  private List<String> userSubEntityList =
      new ArrayList<>(
          Arrays.asList(
              JsonKey.ADDRESS, JsonKey.JOB_PROFILE, JsonKey.EDUCATION, JsonKey.EXTERNAL_IDS));
  ...
  public Response createUser(Request request) throws BaseException {
    ...
    Response userAttributeResponse =
        saveUserAttributes(request, (String) userResponse.get(JsonKey.ID));
    ...
    try {
      ...
      ActorRef actorRef =
          Application.getInstance()
              .getActorRef(UserActorOperations.SAVE_USER_ATTRIBUTES.getOperation());
      ...
      ProjectLogger.log(
          "UserServiceImpl:saveUserAttributes : Exception occurred while saving user sub entity like address,education and job profile details request.",
          ex);
    }
    return null;
  }
  ...
  private void saveESUserData(Response userAttributeResponse, Response userResponse) {
    try {
      ...
      Map<String, Object> userMap =
          (Map<String, Object>) userResponse.getResult().get(StringUtils.capitalize(JsonKey.USER));
      ...
      ActorRef actorRef =
          Application.getInstance().getActorRef(UserActorOperations.SAVE_ES_USER.getOperation());
      esRequest.setOperation(UserActorOperations.USER_ES_CREATE.getOperation());
      actorRef.tell(esRequest, ActorRef.noSender());
    } catch (Exception ex) {
      ProjectLogger.log(
          "UserServiceImpl:saveESUserData : Exception occurred while creating user data request for ES.",
          ex);
    }
  }
}
