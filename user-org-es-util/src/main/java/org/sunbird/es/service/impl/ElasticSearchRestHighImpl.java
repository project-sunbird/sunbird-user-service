...
public class ElasticSearchRestHighImpl implements ElasticSearchService {
  ...
  public Future<String> save(String index, String identifier, Map<String, Object> data) {
    ...
    ElasticSearchHelper.logMethodStart(
        "ElasticSearchUtilRest:save: method started at ==", startTime);
        ...
        new ActionListener<IndexResponse>() {
          ...
          public void onResponse(IndexResponse indexResponse) {
            ...
            ElasticSearchHelper.logMethodEnd(
                "ElasticSearchRestHighImpl:save: method end at ==", startTime);
          }
   ...
   * if data is not valid then this method will throw invalid data exception.
   *
   * @param index String ES index name
   * @param identifier ES column identifier as an String
   * @param data Map<String,Object>
          ...
          new ActionListener<UpdateResponse>() {
            ...
            public void onResponse(UpdateResponse updateResponse) {
              ...
              ProjectLogger.log(
                  ...
                  LoggerEnum.INFO.name());
              ElasticSearchHelper.logMethodEnd(
                  "ElasticSearchRestHighImpl:update: method end at ==", startTime);
            }
  ...
  public Future<Map<String, Object>> getDataByIdentifier(String index, String identifier) {
    ...
    if (StringUtils.isNotBlank(index) && StringUtils.isNotBlank(identifier)) {
      ...
      ProjectLogger.log(
          "ElasticSearchRestHighImpl:getDataByIdentifier: either index or identifier is null or empty "
              + index
              + "-"
              + identifier,
          LoggerEnum.INFO.name());
  ...
  public Future<Boolean> delete(String index, String identifier) {
    ...
    if (StringUtils.isNotBlank(index) && StringUtils.isNotBlank(identifier)) {
      ...
      ElasticSearchHelper.logMethodEnd(
          "ElasticSearchRestHighImpl:delete: method end ==", startTime);
    } else {
      ProjectLogger.log(
          "ElasticSearchRestHighImpl:delete:OnResponse: either index or identifier is null or empty "
              + index
              + "-"
              + identifier,
          LoggerEnum.INFO.name());
      Exception e = new Exception(EsConstant.INVALID_REQUEST_DATA);
      promise.failure(e);
    }
    return promise.future();
  }
    ...
    if (searchDTO.getSortBy() != null && searchDTO.getSortBy().size() > 0) {
      for (Map.Entry<String, Object> entry : searchDTO.getSortBy().entrySet()) {
        if (!entry.getKey().contains(".")) {
          ...
          Map<String, String> dataMap = (Map) map.get(EsConstant.TERM);
          for (Map.Entry<String, String> dateMapEntry : dataMap.entrySet()) {
            FieldSortBuilder mySort =
                new FieldSortBuilder(entry.getKey() + ElasticSearchHelper.RAW_APPEND)
                    ...
                    .order(ElasticSearchHelper.getSortOrder((String) map.get(EsConstant.ORDER)));
    ...
    ActionListener<SearchResponse> listener =
        new ActionListener<SearchResponse>() {
          ...
          public void onFailure(Exception e) {
            promise.failure(e);
            ElasticSearchHelper.logMethodEnd(
                "ElasticSearchRestHighImpl:search:onFailure: method end at ==", startTime);
            ProjectLogger.log(
  ...
  public Future<Boolean> healthCheck() {
    ...
    GetIndexRequest indexRequest = new GetIndexRequest().indices(EsConstant.USER_INDEX_NAME);
    Promise<Boolean> promise = Futures.promise();
  ...
  public Future<Boolean> bulkInsert(String index, List<Map<String, Object>> dataList) {
    ...
    ElasticSearchHelper.logMethodEnd(
        "ElasticSearchRestHighImpl:bulkInsert: method end ==", startTime);
    return promise.future();
  }
  ...
  private static SearchSourceBuilder addAggregations(
    ...
    ElasticSearchHelper.logMethodEnd(
        "ElasticSearchUtilRest:addAggregations: method end ==", startTime);
  ...
  public Future<Boolean> upsert(String index, String identifier, Map<String, Object> data) {
    long startTime = System.currentTimeMillis();
    Promise<Boolean> promise = Futures.promise();
    ElasticSearchHelper.logMethodStart("ElasticSearchUtil:upsert: method started at ==", startTime);
          ...
          new ActionListener<UpdateResponse>() {
            ...
            public void onResponse(UpdateResponse updateResponse) {
              ...
              ProjectLogger.log(
                  ...
                  LoggerEnum.INFO.name());
              ElasticSearchHelper.logMethodEnd(
                  "ElasticSearchUtilRest:upsert:onResponse: method end ==", startTime);
            }
  ...
  public Future<Map<String, Map<String, Object>>> getEsResultByListOfIds(
    ...
    filters.put(EsConstant.ID, ids);

    SearchDTO searchDTO = new SearchDTO();
    searchDTO.getAdditionalProperties().put(EsConstant.FILTERS, filters);
    ...
    List<Map<String, Object>> esContent =
        (List<Map<String, Object>>) result.get(EsConstant.CONTENT);
